...
const accidentLayer = new ol.layer.Vector({
    ...
    style: function(feature) {
        ...
        const typeMatch = selectedType === 'All' || feature.get('accident_type') === selectedType;
        const severityMatch = selectedSeverity === 'All' || feature.get('severity') === selectedSeverity;
        return (typeMatch && severityMatch) ? accidentStyleFunction(feature) : null;
    }
});
...
const overlay = new ol.Overlay({ element: popupContainer, positioning: 'bottom-center', stopEvent: false });
...
// Switchers
document.getElementById('osm').addEventListener('change', function() { osmLayer.setVisible(this.checked); });
document.getElementById('google-sat').addEventListener('change', function() { googleSatelliteLayer.setVisible(this.checked); });
document.getElementById('boundary').addEventListener('change', function() { boundaryLayer.setVisible(this.checked); });
document.getElementById('accidents').addEventListener('change', function() { accidentLayer.setVisible(this.checked); });
...
accidentForm.addEventListener('submit', function(e) {
    ...
    if (!reportLocation) {
        alert("Location missing. Click 'Add Report' first.");
        return;
    }
